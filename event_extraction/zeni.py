...
#     1. The foot is moving downward (positive vertical velocity)
#     2. Forward velocity of the foot decreases sharply (negative acceleration in x)
#     3. The foot reaches its lowest vertical position relative to the pelvis (local maximum in y, since y increases downward)
...
#         # Filter out errors using error_mask
#         if error_mask is not None:
#             valid_mask = ~error_mask[paw_name]
#             x_valid = x_rel.copy()
#             y_valid = y_rel.copy()
#             x_valid[~valid_mask] = np.nan
#             y_valid[~valid_mask] = np.nan
#         else:
#             x_valid = x_rel.copy()
#             y_valid = y_rel.copy()
        
#         # Fill NaN values: forward fill first, then backward fill
#         x_filled = x_valid.ffill().bfill()
#         y_filled = y_valid.ffill().bfill()
        
#         # Check if we have valid data
#         if y_filled.isna().all() or x_filled.isna().all():
#             print(f'No valid data available for {paw_name}')
#             continue
        
#         # Smooth the signals to reduce noise
#         x_smooth = pd.Series(
#             uniform_filter1d(x_filled.values, size=window_size),
#             index=x_valid.index
#         )
#         y_smooth = pd.Series(
#             uniform_filter1d(y_filled.values, size=window_size),
#             index=y_valid.index
#         )
...
#         vertical_velocity = y_smooth.diff()
        
#         # Forward velocity: change in x position
#         forward_velocity = x_smooth.diff()
...
#             vel_before = vertical_velocity.iloc[max_idx - 1]
