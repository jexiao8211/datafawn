"""
Implementation of soundscape generation from configuration.

This module contains the core logic for generating soundscapes from event data
...
def generate_soundscape_from_config(
    ...
    """
    Add sounds for detected events to a video based on configuration.
    
    Iterates through all individuals and adds sounds for each event type
    based on the event_sound_map in the config. Also handles backing tracks
    ...
    """
    event_sound_map = soundscape_config['event_sound_map']
    backing_track = soundscape_config.get('backing_track')
    
    # Get volume settings from config, with defaults
    volume_config = soundscape_config.get('volume', {})
    ...
    for (scorer, individual), event_dict in events_dict.items():
        ...
        for event_name, strike_frames in event_dict.items():
            ...
            for strike_time in strike_times:
                clip = create_sound_clip(
                    sound_path, 
                    sound_duration, 
                    strike_time, 
                    video.duration, 
                    volume=event_sounds_volume
                )
                if clip is not None:
